...
public class MyFriendFollowFragment extends BaseFragment {
    ...
    public MyFriendFollowFragment(String lookUserId) {
        LogUtils.show(TAG, "----MyFriendFollowFragment----lookUserId = " + lookUserId);
    ...
    public View initView() {
        View view = View.inflate(mFriendActivity, R.layout.fragment_my_friend_follow, null);
        rvFriendFollow = (RecyclerView) view.findViewById(R.id.rv_friend_follow);
    ...
    private void gotoGetFollowFriend() {
        if (pageNo == 0 && !isPullRefresh) {
            llLoading.setVisibility(View.VISIBLE);
        }
        OkHttpUtils.get().url(GlobalConstants.URL_FRIEND_FOLLOW)
                ...
                .execute(new StringCallback() {
                    ...
                    public void onResponse(String response, int id) {
                        ...
                        try {
                            parserFollowFriendsJson(response);
                        } catch (Exception e) {
                            e.printStackTrace();
    ...
    private void parserFollowFriendsJson(String response) {
        Gson gson = new Gson();
        ...
        if (friendsListData == null || friendsListData.data == null || friendsListData.data.size() == 0) {  //没有好友
            if (pageNo == 0) {
                Toast.makeText(mFriendActivity, "还没有关注的人", Toast.LENGTH_SHORT).show();
                llFriendsHave.setVisibility(View.GONE);
                llFriendsNone.setVisibility(View.VISIBLE);
            } else {
                Toast.makeText(mFriendActivity, "没有更多关注的人了", Toast.LENGTH_SHORT).show();
            }
        } else {  //有好友
            mFriendsList.addAll(friendsListData.data);

            //RecyclerView加载数据
            if (pageNo == 0) { //page:每次加载15条 2018年3月16日取消分页加载
                rvFriendFollow.setLayoutManager(new LinearLayoutManager(mFriendActivity));
                mFriendsAdapter = new FriendsAdapter();
                rvFriendFollow.setAdapter(mFriendsAdapter);
    ...
    private class FriendsAdapter extends RecyclerView.Adapter<FriendsHolder> {
        ...
        public void onBindViewHolder(FriendsHolder holder, int position) {
            ...
            Glide.with(MyFriendFollowFragment.this).load(mFriendsList.get(position).images1).into(holder.ivFriendUserIcon);
            ...
            holder.itemView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    intent.putExtra("LOOK_USER_ID", mFriendsList.get(position).uid);
